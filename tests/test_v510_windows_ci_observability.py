...
def test_windows_build_log_is_streamed_live_and_persisted() -> None:
    ...
    assert 'Tee-Object -FilePath "ci-proof\\build.log"' in text
    assert '*> "ci-proof\\build.log"' not in text
    assert 'Get-Content -LiteralPath "ci-proof\\build.log"' not in text


def test_cancelled_run_uploads_partial_failure_evidence() -> None:
    text = WORKFLOW.read_text(encoding="utf-8")
    assert "if: ${{ failure() || cancelled() }}" in text
    assert "ci-proof/**" in text
