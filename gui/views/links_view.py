...
class LinksView(QWidget):
    def __init__(self, adapter):
        ...
        subtitle = QLabel(
            "LansetSpBot найдёт чат обсуждения, вступит при необходимости и сохранит готовую связку."
        )
        ...
        info = QLabel(
            "Программа получает ID обсуждения через Telegram API, один раз проверяет "
            "участие и при необходимости вступает без повторной проверки участия. "
            "Между проверками выдерживается случайная пауза "
            "3–7 секунд, между новыми вступлениями — 15–25 секунд. Каждый канал "
            "и чат обсуждения проверяется только один раз. Кнопка «Стоп» не сокращает "
            "FloodWait: вызвавший его канал помечается пропущенным, задача дождётся "
            "Telegram-таймера и защитного буфера, затем продолжит со следующего объекта "
            "или останется на паузе до кнопки «Продолжить связки»."
        )
